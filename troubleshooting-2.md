...
``` r
### ERROR HERE ###
#fixed from load.packages() to library() to load pacakges, and install.packages for packages not yet installed
library(dslabs)
library(tidyverse)
```
...
``` r
#changed "==" to ">" as the criteria asks for movies filmed AFTER 2000
...
``` r
### ERROR HERE ###
#removed the columns following titles and genres, and replaced with "Everything"
movieLens %>%
  filter(!genres == "Drama") %>%
...
``` r
### ERROR HERE ###
#removed the second equal signs as we only need one
movieLens <- movieLens %>%
...
``` r
### ERROR HERE ###
#added group_by, changed mutate to summarize
movieLens %>%
  group_by(title) %>%
...
``` r
### ERROR HERE ###
#specify height and mass with vectors
starWars %>%
  group_by(species) %>%
...
#added comma after birth_location, added quotations for locations
```
